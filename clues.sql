...
\connect world
SELECT * FROM countries WHERE region = 'Southern Europe' ORDER BY countries.population;
...
WHERE countrylanguages.countrycode = 'VAT'
AND isofficial = TRUE;
...
FROM countries, countrylanguages WHERE countries.code = countrylanguages.countrycode
...
where countries.code = 'ITA' limit 1;

-- Clue #5: Oh no, she pulled a switch – there are two cities with very similar names, but in totally different parts of the globe! She's headed to South America as we speak; go find a city whose name is like the one we were headed to, but doesn't end the same. Find out the city, and do another search for what country it's in. Hurry!

-- Write SQL query here
SELECT cities.name
FROM countries JOIN cities ON countries.code = cities.countrycode
WHERE countries.region = 'South America' ORDER BY cities.name limit 1;
...
--               In a city of ninety-one thousand and now, eighty five.91000 85
